...
function Pack(container, options) {
  ...
  this.classes = classes(container)
...
Pack.prototype.append = function () {
...
Pack.prototype.create = function () {
  ...
  var parts = part(ratios, Math.ceil(ratios.reduce(add, 0) / this.width * this.height))
  ...
  parts.forEach(function (part) {
    index += this.createRow(index, part.length)
  }, this)
...
Pack.prototype.createRow = function (index, count) {
  ...
  var height = Math.min(this.calculateRowHeight(images), this.maxheight)
