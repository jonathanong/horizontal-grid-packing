...
Pack.prototype.destroy = function () {
  ...
  container.className = container.className.replace(/\bhor-pack\b/gi, '')
...
Pack.prototype.create = function () {
  ...
  while (index < this.images.length)
    index += this.createRow(index)

  var lastmirror = mirrors[mirrors.length - 1]

  if (!container.className.match(/\bhor-pack\b/gi))
    container.className += ' hor-pack'

  container.style.height = (lastmirror.top + lastmirror.height) + 'px'
  container.style.visibility = 'visible'
}

Pack.prototype.createRow = function (index) {
